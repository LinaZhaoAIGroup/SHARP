...
class CompoundLoss(_Loss):
    ...
    def __init__(self, blocks=[1, 2, 3, 4], mse_weight=0, resnet_weight=1,ssim_weight=1,device='cpu'):
    ...
    def forward(self, input, target):
        ...
        if self.resnet_weight!=0:
            ...
            if self.mse_weight!=0 and self.ssim_weight!=0:
                loss = self.mse_weight * self.criterion(input, target) + self.resnet_weight * loss_value+self.ssim_weight*self.SSIMloss(input, target)
            elif self.mse_weight!=0 and self.ssim_weight==0:
                loss = self.mse_weight * self.criterion(input, target) + self.resnet_weight * loss_value
            elif self.mse_weight==0 and self.ssim_weight!=0:
                loss = self.resnet_weight * loss_value+self.ssim_weight*self.SSIMloss(input, target)
            elif self.mse_weight==0 and self.ssim_weight==0:
                loss =self.resnet_weight * loss_value
        elif self.resnet_weight==0:
            if self.mse_weight!=0 and self.ssim_weight!=0:
                loss = self.mse_weight * self.criterion(input, target) +self.ssim_weight*self.SSIMloss(input, target)
            elif self.mse_weight!=0 and self.ssim_weight==0:
                loss = self.mse_weight * self.criterion(input, target)
            elif self.mse_weight==0 and self.ssim_weight!=0:
                loss = self.ssim_weight*self.SSIMloss(input, target)
        return loss